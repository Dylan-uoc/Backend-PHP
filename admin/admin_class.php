...
Class Action {
	...
	function login_student(){
		...
		extract($_POST);		
		$qry = $this->db->query("SELECT *,concat(lastname,', ',firstname) as name FROM student where nif = '".$nif."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
	...
	function save_student(){
		...
		foreach($_POST as $k=> $v){
			...
			if(empty($nif)){
				...
				while($i == 1){
					...
					$chk = $this->db->query("SELECT * FROM student where nif = '$rand' ")->num_rows;
					if($chk <= 0){
						$data .= ", nif='$rand' ";
						$i = 0;
					}
		...
		if(empty($id)){
			if(!empty($nif)){
				$chk = $this->db->query("SELECT * FROM student where nif = '$nif' ")->num_rows;
				if($chk > 0){
					return 2;
					exit;
				}
			...
			if(!empty($nif)){
				$chk = $this->db->query("SELECT * FROM student where nif = '$nif' and id != $id ")->num_rows;
				if($chk > 0){
					return 2;
					exit;
				}
	...
	function get_schecdule(){
		extract($_POST);
		$data = array();
		$qry = $this->db->query("SELECT * FROM schedules where student_id = 0 or student_id = $student_id");
